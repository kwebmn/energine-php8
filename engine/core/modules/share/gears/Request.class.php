...
final class Request extends BaseObject
{
    ...
    /** «uk/» либо '' */
    public function getLangSegment(): string
    {
    ...
    public function getPath(int $what = self::PATH_WHOLE, bool $asString = false): array|string
    {
        switch ($what)
        {
            case self::PATH_TEMPLATE: $path = array_slice($this->path, 0, $this->offset);
                break;
            case self::PATH_ACTION:   $path = array_slice($this->path, $this->offset);
                break;
            default:                  $path = $this->path;
                break;
        }
    ...
    public function setPathOffset(int $offset): void
    {
        ...
        $this->useSegments($this->offset);
    }
    public function shiftPath(int $offset): void
    {
    ...
    public function useSegments(int $count = 1): void
    {
        $this->usedSegmentsCount = max(0, $count);
    }
    public function getUsedSegments(): int
    {
    ...
    {
        return $this->sreq->getHost();
    }
    public function getScheme(): string
    {
        return $this->sreq->getScheme();
    }
    public function getPort(): int
    {
        return (int)$this->sreq->getPort();
    }

    public function getUserAgent(): string
    {
        return (string)$this->sreq->headers->get('User-Agent', '');
    }
    public function getReferrer(): string
    {
        return (string)$this->sreq->headers->get('Referer', '');
    }

    public function getClientIps(): array
    {
        return $this->sreq->getClientIps();
    }
