...
<xsl:stylesheet
    ...
    <xsl:variable name="UI">
        <xsl:choose>
            <xsl:when test="normalize-space($DOC_PROPS[@name='ui']/@value) != ''">
                <xsl:value-of select="normalize-space($DOC_PROPS[@name='ui']/@value)"/>
            </xsl:when>
            <xsl:when test="normalize-space($DOC_PROPS[@name='ui']) != ''">
                <xsl:value-of select="normalize-space($DOC_PROPS[@name='ui'])"/>
            </xsl:when>
            <xsl:otherwise>bootstrap5</xsl:otherwise>
        </xsl:choose>
    </xsl:variable>
    ...
    <xsl:template match="/document">
        <html>
        	<head>
                ...
                />
                <xsl:choose>
                    <xsl:when test="$UI = 'mdbootstrap'">
                        <link
                                href="{concat($ASSETS_BASE, 'energine.mdvendor.css')}"
                                rel="stylesheet"
                        />
                    </xsl:when>
                    <xsl:otherwise>
                ...
                <xsl:choose>
                    <xsl:when test="$UI = 'mdbootstrap'">
                        <script
                                ...
                                src="{concat($ASSETS_BASE, 'energine.mdvendor.js')}"
                                defer="defer"
                        ></script>
                    </xsl:when>
                    <xsl:otherwise>
