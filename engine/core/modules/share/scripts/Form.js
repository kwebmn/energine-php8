...
class FormRichEditor {
    ...
    constructor(textarea, form) {
        ...
        /**
         * @type {Promise<tinymce.Editor|null>|null}
         */
        this.initPromise = null;

        if (!tinymce) {
            console.warn('TinyMCE is not available in the global scope.');
            return;
        }
    ...
    /**
     * Подготавливает стили TinyMCE (однократно на проект)
     */
    ensureStyles() {
        if (!FormRichEditor.styles) {
            FormRichEditor.styles = collectWysiwygStyles();
        }
    }
    ...
    initEditor() {
        ...
        if (!this.textarea.id) {
            this.textarea.id = `tinymce-editor-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
        }

        const setupEditor = (editor) => {
            this.editor = editor;
            editor.editorId = this.textarea.id;
            editor.singleTemplate = this.form?.singlePath || '';
        };

        const rows = parseInt(this.textarea.getAttribute('rows') || '0', 10);
    ...
    onSaveForm() {
        ...
        if (this.editor) {
            try {
                this.textarea.value = this.editor.getContent({ format: 'html' });
            } catch (e) {
                console.warn(e);
            }
            return;
        ...
        if (this.initPromise) {
            this.initPromise.then((editor) => {
                if (editor && editor === this.editor) {
                    try {
                        this.textarea.value = editor.getContent({ format: 'html' });
                    } catch (e) {
                        console.warn(e);
                    }
                }
