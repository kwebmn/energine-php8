...
| `FileRepository.js` | `FileRepository` extends `GridManager` | `GridManager`, `Cookie`, `FileAPI/FileAPI` | Uses FileAPI plugin globals, `ModalBox`, DOM drag/drop.【F:engine/core/modules/share/scripts/FileRepository.js†L1-L60】 |
| `FileRepoForm.js` | `FileRepoForm` extends `Form` | `Form`, `FileAPI/FileAPI` | Upload helpers built on FileAPI shim.【F:engine/core/modules/share/scripts/FileRepoForm.js†L1-L40】 |
...
| `AttachmentEditor.js` | `AttachmentEditor` extends `GridManager` | `GridManager`, `FileAPI/FileAPI` | Handles repository attachments with FileAPI globals.【F:engine/core/modules/share/scripts/AttachmentEditor.js†L1-L60】 |
...
- CKEditor, FileAPI, jsTree, jQuery, Bootstrap – shipped as vendor globals and consumed directly (`CKEDITOR`, `$`, `bootstrap`). Font Awesome icons are bundled with the public theme for Energine UI helpers.【F:engine/core/modules/share/scripts/Form.js†L1-L64】【F:engine/core/modules/share/scripts/DivManager.js†L1-L60】【F:engine/core/modules/share/scripts/TemplateWizard.js†L21-L32】【F:site/modules/default/transformers/energine.xslt†L52-L88】
...
- **MIME types:** All scripts are currently served as `.js` through relative URLs such as `scripts/default/bootstrap.bundle.min.js` and `{$STATIC_URL}scripts/{path}.js`. When switching to native modules ensure the HTTP server continues to return `text/javascript` for both `.js` and future `.mjs` files (no conflicting configuration found in the repository).【F:site/modules/default/transformers/energine.xslt†L61-L88】【F:engine/core/modules/share/transformers/document.xslt†L248-L260】
