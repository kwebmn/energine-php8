...
- **engine/vite/vendor/ckeditor/** (ядро редактора; кастомные плагины Energine лежат в `engine/core/modules/share/scripts/ckeditor/plugins`), **vendor/codemirror/codemirror/** (Composer-managed), **fancytree/**, **jstree/**, and `jquery.min.js` – vendor sources kept as global scripts and now referenced directly from component XML via `<javascript><library loader="classic">…</library></javascript>`. These remain untouched during the ES-module migration and continue to expose their historical globals (`CKEDITOR`, `CodeMirror`, `jQuery`, etc.).【3db03a†L12-L110】【935dbc†L1-L120】
