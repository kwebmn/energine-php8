...
3. **Инициализационный код**
   ...
   - Для `Energine.js` рантайм сохраняет автоинициализацию внутри модуля: файл самостоятельно считывает `dataset`, вызывает `Energine.boot()` и при необходимости закрепляет результат в `window.Energine`. Отдельный экспорт стартовой функции не требуется.
   - Вспомогательные экспорты для ручной передачи конфигурации (`bootEnergine`, `createConfigFromProps`) признаны избыточными и удалены; XSLT ограничивается расстановкой `data-*` атрибутов.
...
4. **Шаблоны `document.xslt` и `energine.xslt`**
   - В `engine/core/modules/share/transformers/bootstrap/document.xslt` и `site/modules/default/transformers/energine.xslt` обновить блоки, отвечающие за старт платформы:
     ...
     - удалить устаревший `START_ENERGINE_JS` и любые inline-скрипты, которые вручную вызывали рантайм; модуль `Energine.js` выполняет запуск самостоятельно.
   ...
   - Дополнить шаблоны комментариями о том, что ручные вызовы рантайма больше не нужны; достаточно подключить модуль и расставить `data-e-*` атрибуты.
