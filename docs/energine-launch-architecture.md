...
После внедрения нового ядра `Energine.js` предоставляет тонкий программный реестр `Energine.register(name, value)` и
`Energine.resolve(name)` для экспонирования/получения классов поведения без обращения к `window['ClassName']`. UI- и сетевые
помощники вынесены в отдельные модули `energine-ui.js` и `energine-network.js`; страницы подключают их по мере необходимости,
что позволяет tree shaking исключать неиспользуемые надстройки из базового бандла.
