...
3. **Размещение сценариев**
   ...
   - После загрузки DOM сам модуль считывает `dataset` подключившего его `<script>` и проводит инициализацию. В шаблонах запрещены inline-вызовы `bootEnergine()` или другие инструкции, которые вручную запускают рантайм, как это делается сейчас внутри `document.xslt` через `<script type="module">` блок с импортами и ручными вызовами (включая устаревший `attachToWindow`). Этот блок подлежит удалению после переноса запуска внутрь модуля.【F:engine/core/modules/share/transformers/bootstrap/document.xslt†L212-L304】
   - Дополнительные модули подключаются аналогично как ES6-модули либо входят в собранный бандл.

4. **Маркировка клиентских классов**
   - Каждый интерактивный блок, которому требуется клиентский класс, получает атрибут `data-e-js="<ClassName>"` на корневом элементе компонента. Значение `<ClassName>` берётся из `@sample` соответствующего `*.component.xml` (behavior-описание), а не из PHP-классов, чтобы сохранялось совпадение с текущими декларациями поведения.
   - Дополнительные настройки экземпляра передаются через остальные `data-*` атрибуты того же элемента (например, `data-e-mode="grid"`, `data-e-source="toolbar"`).
   - Идентификаторы, генерируемые исключительно ради привязки JS (`generate-id()` в XSLT), устраняются. Атрибут `id` сохраняется только там, где он необходим для ARIA, лейблов форм или ссылок. Текущие вызовы `generate-id` в шаблонах `document.xslt`, `list.xslt`, `form.xslt` и других служат исходной точкой для замены на `data-e-js` и декларативные параметры.【F:engine/core/modules/share/transformers/bootstrap/document.xslt†L205-L259】【F:engine/core/modules/share/transformers/bootstrap/list.xslt†L32-L160】【F:engine/core/modules/share/transformers/bootstrap/form.xslt†L13-L170】
...
1. **Точка входа**
   ...
   - Автоматический запуск перенесён в сам модуль: `Energine.js` определяет `<script type="module">`, из которого он был загружен, считывает его `dataset` через `readConfigFromScriptDataset()` и самостоятельно вызывает `Energine.boot()` без участия inline-кода. Текущий bootstrap в `document.xslt`, импортирующий рантайм и вручную создающий экземпляр, подлежит удалению после переноса логики внутрь модуля.【F:engine/core/modules/share/transformers/bootstrap/document.xslt†L205-L304】【F:engine/core/modules/share/scripts/Energine.js†L1-L260】

2. **Автоподключение компонент через `data-e-js`**
   - После выполнения `boot()` модуль `Energine.js` один раз обходит DOM в поисках элементов с атрибутом `data-e-js`. Для каждого элемента извлекается имя класса инициализации, доступное из `globalThis` (или зарегистрированное ранее через экспорт модуля).
   - Базовый алгоритм минималистичен: `const ClassRef = globalThis[className]; if (typeof ClassRef === 'function') new ClassRef(element); else safeConsoleError(...)`. Никаких промежуточных реестров, очередей задач и конструкторов-оберток не требуется.
   - Чтобы исключить повторное создание экземпляров при повторной инициализации, `Energine.js` помечает обработанный элемент (`element.__energineInstance = instance` или `element.dataset.eJsApplied = '1'`). Повторный обход (например, при ленивом подключении) пропускает уже обработанные узлы.
   - При необходимости передачи параметров класс считывает их из `element.dataset`, поэтому XSLT подготавливает описательные `data-*`. Никаких JSON-блоков или сериализованных конфигураций в шаблонах не создается.
   - Подключение административных тулбаров и прочих behavior-классов, которые сейчас создаются вручную через inline-скрипт и `generate-id`, переходит на эту схему. После удаления скрипта из `document.xslt` обработка `PageToolbar`, `PageEditor` и остальных модулей выполняется автоматически при наличии соответствующего `data-e-js` на контейнере.【F:engine/core/modules/share/transformers/bootstrap/document.xslt†L224-L259】
...
4. Убедиться, что глобальный доступ предоставляется автоматически самим модулем (`Energine` экспортирует инстанс и закрепляет
   его в `window`), и удалить устаревшие адаптеры, завязанные на ручные вызовы `attachToWindow`.【F:engine/core/modules/share/scripts/Energine.js†L39-L59】【F:engine/core/modules/share/scripts/Energine.js†L874-L876】
...
- Инициализация компонент зависит от `generate-id()` и присвоения экземпляров в `globalThis['<id>']`, что требует уникальных `id` даже там, где они не нужны по семантике. Переезд на `data-e-js` ликвидирует эти искусственные идентификаторы и упрощает запуск behavior-классов.【F:engine/core/modules/share/transformers/bootstrap/document.xslt†L220-L259】【F:engine/core/modules/share/transformers/bootstrap/list.xslt†L32-L160】
