...
- **`energine.extended.vendor.entry.js`** – подборка тяжёлых библиотек, необходимых только авторизованным (CKEditor, jsTree, CodeMirror 5.65.16 и т.д.). И jQuery, и jsTree подключаются из Composer-пакетов, а в репозитории остаются только лёгкие заглушки для режима отладки. jsTree и CodeMirror устанавливаются из npm-архивов через Composer и импортируются напрямую из `vendor/components/...`.
