...
- **Пакетные зависимости** – достаточно `vite` в `devDependencies`. Дополнительные инструменты не требуются.

---

## 2. Как запустить сборку

1. Установите зависимости (при первом запуске либо после обновления `package.json`):
   ```bash
   npm install
   ```
2. Соберите клиентские ресурсы:
   ```bash
   npm run build
   ```
...
3. Проверьте содержимое каталога `assets/`. Первый бандл очищает каталог, остальные дописываются в существующие файлы.
...
- **Пути**: экспортируются переменные `rootDir`, `repoRoot`, `entriesDir`, `outputDir`, `engineDir`, `siteDir`. Благодаря этому все entry-файлы и build-скрипты могут оперировать абсолютными путями независимо от текущего каталога.
...
- **`energine.vendor.entry.js`** – минимальный набор для всех пользователей: jQuery и Bootstrap. Следит за тем, чтобы глобальные `window.$` и `window.jQuery` были заданы даже при модульной загрузке.
- **`energine.extended.vendor.entry.js`** – подборка тяжёлых библиотек, необходимых только авторизованным (CKEditor, FileAPI, jsTree, CodeMirror и т.д.).
