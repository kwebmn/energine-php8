...
3. **Реестр behavior-классов.**
   ...
   - В debug-режиме (атрибут `<script data-debug="true">`) `scanForComponents` выбрасывает `Error`, если компонент встретил неизвестное значение `data-e-js`. Это помогает найти забытые импорты/регистрации во время миграции. В продакшене рантайм продолжает повторять инициализацию до тех пор, пока модуль не будет загружен.
