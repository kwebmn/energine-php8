...
## Добавление нового встроенного компонента

Ниже приведён единый алгоритм, который можно применять в административных
компонентах любого уровня — от простого `DataSet` до сложных редакторов наподобие
`SiteEditor`.

1. **Определите состояние и маршруты.** Выберите уникальное имя состояния и, при
   необходимости, опишите URI-паттерны в `getModalRoutePatterns()`. Например, `Grid`
   сопоставляет состояние `attachments` с маршрутами `attachments/` и
   `attachments/(\d+)/`, что позволяет `GridConfig` автоматически настроить
   роутер.【F:engine/core/modules/share/components/Grid.class.php†L100-L173】【F:engine/core/modules/share/gears/GridConfig.class.php†L15-L25】
2. **Расширьте `registerModals()`.** Добавьте новую запись в возвращаемый массив.
   Для простых редакторов достаточно передать массив с модулем/классом. Так,
   `DataSet` объявляет менеджер изображений при помощи описания
   `['module' => 'share', 'class' => 'ImageManager']`, и базовый класс сам создаёт
   компонент.【F:engine/core/modules/share/components/DataSet.class.php†L37-L129】
3. **Используйте замыкание для сложной подготовки.** Если компонент требует
   вычисления параметров, смещения URI или передачи нестандартных зависимостей,
   верните замыкание. Внутри можно вызвать `activateModalComponent()` и передать
   любые параметры. Пример из `DivisionEditor` показывает, как сместить путь и
   запустить редактор переводов с кастомной конфигурацией.【F:engine/core/modules/share/components/DivisionEditor.class.php†L22-L80】
4. **Работайте с произвольными `IBlock`.** Замыкание может вернуть уже готовый
   контейнер, как в редакторах лент (`TestfeedFeedEditor`, `DefaultTemplateFeedEditor`).
   Это удобно, когда нужно встроить `ComponentContainer` или другой блочный
   объект без дополнительной обёртки.【F:engine/core/modules/auto/components/TestfeedFeedEditor.class.php†L13-L92】【F:engine/core/modules/wizard/components/DefaultTemplateFeedEditor.class.php†L13-L86】
5. **Проверяйте `build()`.** В большинстве случаев дополнительных правок не
   требуется: если состояние объявлено в реестре, `Component::build()` вернёт
   результат дочернего блока автоматически. Исключения — только для компонентов,
   которым нужен особый постпроцессинг (см. обработку `showPageToolbar` в
   `DivisionEditor`).【F:engine/core/modules/share/components/DivisionEditor.class.php†L213-L249】

Следуя этим шагам, новый встроенный компонент появится в административном UI без
дублирования роутов и ручного вызова дочерних обработчиков.
