...
* `icon` — CSS-класс иконки Font Awesome (например, `fa fa-plus`). Отрисовывается в XSLT (`engine/core/modules/share/transformers/components/navigation/toolbar.xslt`).
...
Эта документация охватывает основные концепции конфигов компонентов. За подробностями смотрите исходный код классов `Component`, `Toolbar`, `DataDescription`, а также XSLT-шаблоны в `engine/core/modules/share/transformers/components/`.
