...
- В `engine/core/modules/share/transformers/` точкой входа служит файл `include.xslt` в корне каталога. Он подключает общие правила из `bootstrap/common/`, корневые шаблоны (`bootstrap/document.xslt`) и компонентные файлы из `bootstrap/components/` (например, `bootstrap/components/navigation/toolbar.xslt`). При необходимости дополнительные UI-стеки подключаются собственными `include.xslt` внутри подкаталогов.
