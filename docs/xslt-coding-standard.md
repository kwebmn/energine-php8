...
1. **Корневой уровень модуля**
   - Все трансформеры ядра располагаются в `engine/core/modules/<module_name>/transformers/`.
   - На верхнем уровне держим точку входа `document.xslt` для корневого узла `/document` и рядом с ней — специализированные файлы того же уровня данных: `field.xslt`, `recordset.xslt`, `items.xslt` и т. п. Это упрощает навигацию и сохраняет общий контекст обработки.

2. **Компоненты пользовательского интерфейса**
   - Шаблоны, которые сопоставляются с `component[@name='…']` или `component[@class='…']`, выносим в подкаталог `components/`.
   - Файлы в каталоге `components/` именуем строчными буквами без пробелов и подчёркиваний, отражая назначение (`components/textblock.xslt`, `components/sharebutton.xslt`).
   - Разные варианты отображения одного компонента держим в одном файле и разделяем через режимы (`mode`) или именованные шаблоны.

3. **Общие вспомогательные правила**
   - Нейтральные к UI подготовительные и фильтрующие шаблоны складываем в `common/` или `helpers/` и подключаем через `xsl:include` / `xsl:import`.
   - Такие файлы не должны ссылаться на конкретный стек оформления, чтобы их можно было переиспользовать в других трансформерах.

4. **UI-стеки (Bootstrap, MDBootstrap и др.)**
   - Для каждой поддерживаемой библиотеки создаём подкаталог внутри `transformers/` (`transformers/bootstrap/`, `transformers/mdbootstrap/` и т. д.), повторяющий базовую структуру корневого уровня: `document.xslt`, компонентные файлы и вспомогательные блоки.
   - В каталоге стека не создаём дополнительных уровней вложенности, кроме случаев, когда требуется подкаталог `components/` для сложных UI-компонентов.
   - Центральный файл `include.xslt` в корне модуля агрегирует общие правила (`bootstrap/base.xslt`), корневые шаблоны (`bootstrap/document.xslt`) и компоненты (`bootstrap/list.xslt`, `bootstrap/toolbar.xslt`).

5. **Поддержка и гигиена каталога**
   - Каждый файл начинается с краткого комментария о назначении, ожидаемом контексте и точках расширения.
   - Устаревшие или неиспользуемые файлы (например, `media.xslt`) удаляем. Если нужен функциональный аналог, создаём современный компонент в соответствующем каталоге стека.
